...
I have created a backend api which will send the response in form of JSON, by using modular coding approach which contains having different files for the api connection, the database should be handled by another file.
...
Before running this application, we must have the following softwares:
...
```bash
 npm run start
```
- Open any Browser and search localhost:3000
...
## Developer
  ...
  This project is made solely by Mr. Harshit Sethi
